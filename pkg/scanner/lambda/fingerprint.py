...
# Patterns for parking pages / "for sale" pages
_PARKING_PATTERNS = [
    # NameSilo own parking
    r"namesilo\.com",
...
def _word_count(text: str) -> int:
    return len(re.findall(r"\w{3,}", text))  # words of 3+ characters
...
def extract(domain: str, resp_url: str, status: int, headers: dict,
    ...
    # Final classification (priority top to bottom)
    if is_parking or parking_service:
        ...
        page_type = "active_with_forms"   # potential phishing
    elif word_count > 200:
        page_type = "active_content"
    else:
        page_type = "low_content"

    # Additional headers for clustering
    ...
    # Title hash for exact clustering of identical placeholder pages
    title_md5 = hashlib.md5(title_lower.encode()).hexdigest()[:16] if title_lower else ""

    # Server fingerprint: server + content-type + x-powered-by
